...
static const char *TAG = "watchface_app";
 ...
 */
static lv_obj_t *watchface_app_create(void) {
...
static void watchface_app_on_show(void) {
  ...
  // Update UI with current values (protected by LVGL lock)
  lvgl_port_lock(-1);
  update_time_display(&time);
  update_battery_display();
  update_progress_display();
...
static void watchface_app_on_hide(void) {
...
static void watchface_app_on_gesture(lv_dir_t direction) {
  navigation_manager_handle_gesture(direction);
}

static const app_descriptor_t watchface_descriptor = {
    .id = APP_WATCHFACE,
    .type = APP_TYPE_WATCHFACE,
    .name = "Watchface",
    .icon = NULL,
    .create = watchface_app_create,
    .destroy = NULL,
    .on_show = watchface_app_on_show,
    .on_hide = watchface_app_on_hide,
    .on_gesture = watchface_app_on_gesture,
};

const app_descriptor_t *watchface_app_get_descriptor(void) {
